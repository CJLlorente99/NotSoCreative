...
def show_metrics(data_n, var):
    ...
    a = f.add_subplot(111)
    a.plot(data_n.index, data_n[var])
    return f
...
#   Show metrics (PV, InvestedMoney ...) for our test time window
# def show_metrics(data_n):

# f = Figure(figsize=(5, 5), dpi=100)
# a = f.add_subplot(111)
# a.plot(data_n["MPV"], data_n["MonveyInvestedToday"])

# fig_2 = mpf.plot(data_n, type='line', figsize=(6, 3.5))
# fig_1, axl = mpf.plot(data_n.iloc[-i:, :], type='candle', volume=False,
# style=mpf_style, returnfig=True, addplot=apd, figsize=(6, 3.5))
# return plt
...
def update():
    ...
    if MoneyInvestedToday > 0:
        ...
        labelL2 = ctk.CTkLabel(recommendation_frame, text='Buy', font=ctk.CTkFont(size=20, weight='bold'),
        ...
        # labelL1.grid(row=0, column=1, padx=10, pady=10, sticky="")
        labelL1.pack(side=TOP, padx=10, pady=(10, 5))
        labelL2 = ctk.CTkLabel(recommendation_frame, text='Sell', font=ctk.CTkFont(size=20, weight='bold'),
    ...
    data_n = NewDayValue[["Date", "Open", "High", "Low", "Close", "Volume", "TotalPortfolioValue", "MPV", "absGain", "perGain",
         "MoneyInvestedToday"]]
    data_n
    ...
    label_v_2 = ctk.CTkFrame(tabview.tab("TotaPortfolioValue"), width=800, height=500)
