...
def show_graph_test(data_csv, stock_data, date_test):
    ...
    action = []
    maximum = max(stock_data.iloc[i_date:, :]['High'])
    minimum = min(stock_data.iloc[i_date:, :]['Low'])
    ...
    colors_apd = [get_Color_Buy_Up() if v == a1 else get_Color_Hold() if v == a2 else get_Color_Sell_Down() for v in
                  stock_data["Decision"].iloc[i_date:]]

    apd = mpf.make_addplot(stock_data["Decision"].iloc[i_date:], type='scatter', marker='^', markersize=30,
                           color=colors_apd,
                           panel='main', ylim=(minimum - 50, maximum + 50))

    # plot candelsticks
    mpf_style = mpf.make_mpf_style(base_mpf_style=mode, marketcolors=color_candels)
    fig, axi = mpf.plot(stock_data.iloc[i_date:, :], type='candle', volume=False, style=mpf_style, returnfig=True,
                        addplot=apd,
                        figsize=(6, 4.5))
    # figsize=(0.7*6/100, 0.2*4.5/100))
    axi[1].text(10.6, a3, 'Buy: Green\Purple\nHold: Orange\nSell: Red', style='italic',
                bbox={'facecolor': 'black', 'alpha': 0.5}, fontsize=5)
...
# test
line = FigureCanvasTkAgg(show_graph_test(data_csv, stock_data, date_test), label_test)
line.get_tk_widget().place(width=800, height=350)
# Navigation bar
toolbarFrame = Frame(master=label_test)
toolbarFrame.place(x=0, y=350)
toolbar = NavigationToolbar2Tk(line, toolbarFrame)
