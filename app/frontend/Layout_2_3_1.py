...
#df_charli (nxm) an tag n -> jeden Tag kommt eine neue Zeile: nur eine Strategy
    ##df_charli: (money_inv_today, money_notinv_today, PV)
                #1.day: (x1, y1=10000-x1, 0) ->pvalue=pvalue_current
                #2.day: (x2,y2 , pvalue) at end of the day 1 or pvalue at open price of day 2)

    pvalue_start = 10000
    # charli

    money_inv_today = df_charli['money_inv_today'] #(nx1)
    money_notinv_today = df_charli['money_notinv_today']



    amount = df_charli.Open / money_inv_today #(nx1 on day) (1.5 -1.5 2 0 -1.2)
    amount = np.sum(amount, axis=0) # 1x1 (0.8)

    # update
    money_inv_current = (amount) * df_yf.Close
    pvalue_current = money_notinv_today[-1] + money_inv_current
    gain_current = ((pvalue_start - pvalue_current) / pvalue_start) * 100
    mpvalue_current = pvalue_current + np.sum(df_charli['pvalue']) / df_charli.shape[0]
...
def getCurrentValue_metric(stock_data, data_csv, df_matrix, i_var):
    ...
    return df_matrix.iloc[-1, i_variable]
...
def update():
    ...
    cash = round(getCurrentValue(data_csv, "MoneyNotInvested"), 2)
    stocks = round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "PV") - getCurrentValue(data_csv, "MoneyNotInvested"),
                   2)
    ...
    sidebar_label_2.configure(text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "PV"),2)))
    sidebar_label_4.configure(text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "perGain"), 2)))
    sidebar_label_6.configure(text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "MPV"), 2)))
    ...
    labelL6.configure(text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "StdV"), 2)))
    labelL8.configure(text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "maxGain"), 2)))
    ...
    data_csv = NewDayValue[
        ["Date", "Open", "High", "Low", "Close", "Volume", "TotalPortfolioValue", "MPV", "absGain", "perGain",
         "MoneyInvestedToday", "MoneyNotInvested"]]
    # maybe we should add this to the beinning (Data Prperation)
    # Adding the date column to the dataframe
    lst = []
    for i in range(0, len(data_csv)):
...
cash = round(getCurrentValue(data_csv, "MoneyNotInvested"),2)
stocks = round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "PV")- getCurrentValue(data_csv, "MoneyNotInvested"),2)
...
sidebar_label_2 = ctk.CTkLabel(important_Values_frame,
                               text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "PV"), 2)),
...
sidebar_label_4 = ctk.CTkLabel(important_Values_frame, text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "perGain"), 2)),
                               font=ctk.CTkFont(size=16))
...
sidebar_label_6 = ctk.CTkLabel(important_Values_frame, text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "MPV"), 2)),
                               font=ctk.CTkFont(size=16))
...
labelL6 = ctk.CTkLabel(values_frame, text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "StdV"), 2)),
                       font=ctk.CTkFont(size=16))
...
labelL8 = ctk.CTkLabel(values_frame, text=str(round(getCurrentValue_metric(stock_data, data_csv, df_matrix, "maxGain"), 2)),
                       font=ctk.CTkFont(size=16))
