...
"""
This strategy consists on non investing
"""
class InvestorIdle(Investor):
	...
	def returnBrokerUpdate(self, moneyInvestedToday, moneySoldToday, data: DataManager):
		return pd.DataFrame(
			{'nextStockValue': [data.nextStockValue], 'actualStockValue': [data.actualStockValue],
			 'moneyToInvestIdle': [moneyInvestedToday], 'moneyToSellIdle': [moneySoldToday],
			 'investedMoneyIdle': [self.investedMoney], 'nonInvestedMoneyIdle': [self.nonInvestedMoney]})
	...
	def plotEvolution(self, indicatorData, stockMarketData, recordPredictedValue=None):
		...
		fig.update_layout(
			title="Evolution of Porfolio using idle (" + self.record.index[0].strftime(
				"%d/%m/%Y") + "-" +
