...
class InvestorBaH(Investor):
	...
	def returnBrokerUpdate(self, moneyInvestedToday, data):
		return pd.DataFrame(
			{'actualStockValue': [data["actualStockValue"]],
			 'moneyToInvestBaH': [moneyInvestedToday],
	...
	def plotEvolution(self, expData, stockMarketData, recordPredictedValue=None):
		...
		fig.write_image("images/EvolutionPorfolioBaH(" + self.record.index[0].strftime(
				"%d_%m_%Y") + "-" +
		...
		fig.write_image("images/DecisionMakingBaH(" + self.record.index[0].strftime("%d_%m_%Y") + "-" +
				  self.record.index[-1].strftime("%d_%m_%Y") + ").png",scale=6, width=1080, height=1080)
		# fig.show()