...
# batch 4, n_epoch = 24 backc= 4, 8, 16
...
'''{'batch_size': 2, 'learning_rate': 0.00022394481291754142, 
    'nb_epoch': 35, 'unit1': 150, 'unit2': 100}'''
...
'''Gain sum of prob2: [7.97548101 6.33667875 9.11231257 7.70570047 9.27154222 7.54823202]
Gain sum of prob1: [ 9.06593191  7.11964299  9.77794055  8.29272965 10.10311435  8.79621371]
----------
----------
MPV mean of prob2: [100693.81031283 100664.61479498 100769.82789266 100724.39013989
 100780.74214871 100660.33864332]
MPV mean of prob1: [100789.98488764 100709.95903306 100881.48376592 100776.52262806
 100892.75354936 100797.21862136]'''
...
def fit_model(X_train, y_train, epochs, batch_size):
    ...
    model = build_BimodelHPO(n_inputs, n_features, n_outputs)
...
def main():
    ...
    data_csv = pd.read_csv('featureSelectionDataset_CharliTIs.csv', sep=',', header=0, index_col=0, parse_dates=True,
                           decimal=".")
    ...
    data_list = [1800, 1300, 830, 530, 1400, 750]
    # number of backcandle
    backc = [8, 12, 16]

    # look at these values for optimization
    acc_list2 = np.empty((len(data_list), len(backc)))
    acc_list1 = np.empty((len(data_list), len(backc)))
    bench_list = np.empty((len(data_list), len(backc)))

    gain_list_prob2 = np.empty((len(data_list), len(backc)))
    gain_list_prob1 = np.empty((len(data_list), len(backc)))

    mpv_list_prob2 = np.empty((len(data_list), len(backc)))
    mpv_list_prob1 = np.empty((len(data_list), len(backc)))

    gain_list_best2 = np.empty((len(data_list), len(backc)))
    mpv_list_best2 = np.empty((len(data_list), len(backc)))

    gain_list_best1 = np.empty((len(data_list), len(backc)))
    mpv_list_best1 = np.empty((len(data_list), len(backc)))
    ...
    for c, i in enumerate(data_list):
        ...
        for b, b_c in enumerate(backc):
            ...
            y_pred = inverse_scaling(data, y_pred_scale, scaler_m)
            ...
            if scaling == 2:
                y_pred = inverse_scaling(data, y_pred, scaler_r)
            ...
            for k in range(y_mean.shape[0]):
                ...
                y_test_pd = pd.DataFrame(y_tests[k, :], test_or.iloc[k:k + 1 * y_tests.shape[1]].index,
                                         columns=['Open'])
            ...
            print('----------')
            print('----------')
            acc2 = accuracy_score(real_dec2, decision2)
            print('ACC 2', acc2)
            print('----------')
            print('----------')
            ...
            print('----------')
            print('----------')
            acc1 = accuracy_score(real_dec1, decision1)
            print('ACC 1', acc1)
            print('----------')
            print('----------')
    ...
    print(f'acc2: {acc_list2} %')
    print(f'acc1: {acc_list1} %')
    print(f'acc mean 2: {np.mean(acc_list2, axis=0)}')
    print(f'acc mean 1: {np.mean(acc_list1, axis=0)}')
    print('----------')
    print('----------')
    ...
    print(f'acc mean 2: {np.mean(acc_list2, axis=0)}')
    print(f'acc mean 1: {np.mean(acc_list1, axis=0)}')
    # print(f'Gain sum o rule: {np.sum(gain_list, axis=0)}')
    ...
    # Bidirectional less backcandles better: 8
    # LSTM: 16
    print('Backcandles', backc)
