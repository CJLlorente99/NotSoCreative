...
def main():
    ...
    data = pd.read_csv("featureSelectionDataset_Paul_Class_shift_forBackt_close_open.csv", sep=',', header=0, index_col=0, parse_dates=True,
                       decimal=".")
    data = data.iloc[-600:,:]
    ...
    backtest_func(df=data_or.iloc[-len(y_test):], decision=y_test)
    print('this was for the real value')
    

    backtest_func(df=data_or.iloc[-len(y_test):], decision=y_mean)
