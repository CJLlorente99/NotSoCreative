...
class InvestorLSTMEnsembleClass1(Investor):
	...
	def returnBrokerUpdate(self, moneyInvestedToday, data):
		...
		for i in range(self.n_members):
			aux = pd.concat([aux, pd.DataFrame({"voter" + str(i): data["lstmEnsemble"][i]}, index=[0])], axis=1)

		return pd.concat([aux, pd.DataFrame(
			{'moneyToInvestLSTMEnsembleClass1': moneyInvestedToday,
			 'investedMoneyLSTMEnsembleClass1': self.investedMoney, 'nonInvestedMoneyLSTMEnsembleClass1': self.nonInvestedMoney}, index=[0])], axis=1)

	def possiblyInvestMorning(self, data):
		calculatedData = self.calculateInputsMorning(data['df'])
		self.perToInvest, data['lstmEnsemble'] = self.getPredictionLSTM(calculatedData)

	def possiblyInvestAfternoon(self, data):
		data['lstmEnsemble'] = np.zeros(self.n_members)
	...
	def plotEvolution(self, expData, stockMarketData, recordPredictedValue=None):
		...
		fig.update_layout(
			title="Evolution of Porfolio using LSTM Ensemble 1 (" + self.record.index[0].strftime(
				"%d/%m/%Y") + "-" +
				  self.record.index[-1].strftime("%d/%m/%Y") + ")", xaxis_title="Date",
			yaxis_title="Value [$]", hovermode='x unified')
		fig.write_image("images/EvolutionPorfolioLSTMEnsemble1(" + self.record.index[0].strftime(
				"%d_%m_%Y") + "-" +
		...
		fig.update_layout(
			title="Decision making under LSTM Ensemble 1 (" + self.record.index[0].strftime("%d/%m/%Y") + "-" +
				  self.record.index[-1].strftime("%d/%m/%Y") + ")", hovermode='x unified')
		fig.write_image("images/DecisionMakingLSTMEnsemble1(" + self.record.index[0].strftime("%d_%m_%Y") + "-" +
				  self.record.index[-1].strftime("%d_%m_%Y") + ").png",scale=6, width=1080, height=1080)
		# fig.show()
	...
	def getPredictionLSTM(self, data):
		...
		epochs = 32
		batch_size = 8
		ensemble, y_pred, prob = fit_ensemble1(n_members, X_train, X_test, y_train, y_test, epochs, batch_size)

		clear_session()
		# majority vote, probs for amount
		return majority_vote(y_pred)[0], y_pred[:, 0]

class InvestorLSTMEnsembleClass2(Investor):
	def __init__(self, initialInvestment=10000, n_members=10):
		super().__init__(initialInvestment)
		self.n_members = n_members
	...
	def returnBrokerUpdate(self, moneyInvestedToday, data):
		...
		return pd.concat([aux, pd.DataFrame(
			{'moneyToInvestLSTMEnsembleClass2': moneyInvestedToday,
			 'investedMoneyLSTMEnsembleClass2': self.investedMoney, 'nonInvestedMoneyLSTMEnsembleClass2': self.nonInvestedMoney}, index=[0])], axis=1)
	...
	def plotEvolution(self, expData, stockMarketData, recordPredictedValue=None):
		...
		fig.add_trace(go.Scatter(name="Total Value", x=self.record.index, y=self.record["totalValue"]))
		fig.update_layout(
			title="Evolution of Porfolio using LSTM Ensemble 2 (" + self.record.index[0].strftime(
				"%d/%m/%Y") + "-" +
				  self.record.index[-1].strftime("%d/%m/%Y") + ")", xaxis_title="Date",
			yaxis_title="Value [$]", hovermode='x unified')
		fig.write_image("images/EvolutionPorfolioLSTMEnsemble2(" + self.record.index[0].strftime(
				"%d_%m_%Y") + "-" +
				  self.record.index[-1].strftime("%d_%m_%Y") + ").png",scale=6, width=1080, height=1080)
		# fig.show()
		...
		fig.update_xaxes(title_text="Date", row=2, col=1)
		fig.update_layout(
			title="Decision making under LSTM Ensemble 2 (" + self.record.index[0].strftime("%d/%m/%Y") + "-" +
				  self.record.index[-1].strftime("%d/%m/%Y") + ")", hovermode='x unified')
		fig.write_image("images/DecisionMakingLSTMEnsemble2(" + self.record.index[0].strftime("%d_%m_%Y") + "-" +
				  self.record.index[-1].strftime("%d_%m_%Y") + ").png",scale=6, width=1080, height=1080)
		# fig.show()
	...
	def getPredictionLSTM(self, data):
		...
		epochs = 21
		batch_size = 8
		ensemble, y_pred, prob = fit_ensemble2(n_members, X_train, X_test, y_train, y_test, epochs, batch_size)

		clear_session()
		# majority vote, probs for amount
		return majority_vote(y_pred)[0], y_pred[:, 0]

def class_LSTM1(n_inputs, n_features):
	model = Sequential()
...
def class_LSTM2(n_inputs, n_features):
	opt = Adam(learning_rate=0.001)
	model = Sequential()
	model.add(LSTM(units=170, return_sequences=True, bias_initializer=initializers.Constant(0.01),
...
def fit_model1(X_train, y_train, epochs, batch_size):
	# define neural network model
	n_inputs = X_train.shape[1]
	n_features = X_train.shape[2]
	model = class_LSTM1(n_inputs, n_features)
...
def fit_model2(X_train, y_train, epochs, batch_size):
	# define neural network model
	n_inputs = X_train.shape[1]
	n_features = X_train.shape[2]
	model = class_LSTM2(n_inputs, n_features)
	# fit the model on the training dataset
	early_stopping = EarlyStopping(monitor="accuracy", patience=10, mode='auto', min_delta=0)
	model.fit(X_train, y_train, verbose=2, epochs=epochs, batch_size=batch_size, callbacks=[early_stopping])
	return model

def fit_ensemble1(n_members, X_train, X_test, y_train, y_test, epochs, batch_size):
	ensemble = list()
	y_pred = np.empty((n_members, y_test.shape[0]))
	prob = np.empty((n_members, y_test.shape[0]))
	for i in range(n_members):
		# define and fit the model on the training set
		model = fit_model1(X_train, y_train, epochs, batch_size)
...
def fit_ensemble2(n_members, X_train, X_test, y_train, y_test, epochs, batch_size):
	ensemble = list()
	y_pred = np.empty((n_members, y_test.shape[0]))
	prob = np.empty((n_members, y_test.shape[0]))
	for i in range(n_members):
		# define and fit the model on the training set
		model = fit_model2(X_train, y_train, epochs, batch_size)
		# evaluate model on the test set
		yhat = model.predict(X_test, verbose=2)
		prob[i, :] = yhat.flatten()
