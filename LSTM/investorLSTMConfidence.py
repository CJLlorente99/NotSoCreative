...
class InvestorLSTMConfidenceClass(Investor):
	...
	def possiblyInvestMorning(self, data):
		...
		if sum(data["lstmConfidence"])/self.n_members > 0.5:
			self.perToInvest = 1
		elif sum(data["lstmConfidence"])/self.n_members < 0.5:
			self.perToInvest = -1
	...
	def trainAndPredictMorning(self, df: pd.DataFrame):
		data = pd.DataFrame()
		dataUntilToday = df.copy()
		dataUntilToday['Open'] = dataUntilToday['Open'].shift(-1)
		dataUntilToday = dataUntilToday[:-1]
		# Add indicators
		# Return_close
	...
	def trainAndPredictAfternoon(self, df: pd.DataFrame):
		data = pd.DataFrame()
		dataUntilToday = df.copy()
		# Add indicators
		# Return_close
		...
		epochs = 45
		batch_size = 10
		ensemble, y_pred, accuracy = fit_ensemble(self.n_members, X_train, X_test, y_train, y_test, epochs,
												  batch_size)
		clear_session()
		return y_pred
