...
class InvestorBIA(Investor):
	...
	def possiblyInvestTomorrow(self, data: DataManager):
		...
		"""
		self.moneyToInvest = self.buyPredictionBIA(data.nextStockValueOpen, data.nextStockValueClose)
	...
	def possiblySellTomorrow(self, data: DataManager):
		...
		:param data: Decision data based on the type of indicator
		"""
		self.moneyToSell = self.sellPredictionBIA(data.nextStockValueOpen, data.nextStockValueClose)

	def buyPredictionBIA(self, nextStockValueOpen, nextStockValueClose):
		if nextStockValueClose > nextStockValueOpen:
			return self.nonInvestedMoney
		else:
			return 0

	def sellPredictionBIA(self, nextStockValueOpen, nextStockValueClose):
		if nextStockValueClose < nextStockValueOpen:
